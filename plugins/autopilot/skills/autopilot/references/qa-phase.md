...
3. **Tier 调整**：
   ...
   - Tier 3：聚焦跨任务集成场景（从 design.md 的"跨任务设计约束"提取）
   - Tier 4a：整体架构符合性检查（对照 design.md）
   - Tier 4b：全变更范围代码质量审查
   - Tier 2+：按需
4. **结果判定**：
...
| Tier | 名称 | Wave | 性质 |
|------|------|------|------|
| Tier 0 | 红队契约测试 | Wave 1（并行） | 必须 |
| Tier 1 | 静态验证 | Wave 1（并行） | 必须 |
| Tier 2 | 回归检查 | Wave 1（并行） | 条件（≥3 文件） |
| Tier 3 | 场景验证（含集成健康检查） | Wave 2（串行） | **必须 + 反跳过** |
| Tier 4a-h | AI 审查 | Wave 3（并行 Agent） | 必须/条件 |
| Tier 5 | 性能保障 | 附加 | 不阻塞 |
...
- **`qa_scope: "selective"`**（auto-fix 修复后设置）→ 只重跑上一轮 `### 失败 Tier 清单` 中列出的 Tier + Tier 3，其余 Tier 直接沿用上轮结果标记 ✅
...
**Tier 2: 回归检查**（影响范围跨 3+ 文件时触发）

**执行原则**：遇到失败不中断，标记后继续。记录每项的命令、耗时、退出码、关键输出（前 50 行）。

### Wave 1 失败快速路径（Early Exit to Auto-fix）

Wave 1 完成后统计 Tier 0+Tier 1 ❌ 数量：≥3 → 跳过 Wave 2/3 直接 auto-fix | <3 → 继续 Wave 2 → Wave 3 | auto-fix 后回来执行全量 QA

### Wave 2 — 场景验证（Wave 1 之后，Wave 3 之前，必须执行）

**⚠️ 这是独立的必做步骤，不是 Wave 1 的一部分。Wave 1 所有命令执行完毕后，必须先完成 Wave 2 的全部内容，再启动 Wave 3。**

#### Tier 3 Step 0: 集成健康检查（前置）

验证"系统能跑起来"，是场景执行的环境前提。

**默认规则：Tier 3 必须执行。** 仅当变更满足以下「纯静态排除清单」的**全部**条件时，可标记 N/A：

| 排除条件（必须全部满足） | 说明 |
|--------------------------|------|
| 变更仅涉及文档/注释/README | .md / .txt / 代码注释，无运行时语义 |
| 变更仅涉及 CI 配置文件 | .github/workflows / .gitlab-ci.yml / Dockerfile（不含应用代码） |
| 变更仅涉及版本号/package.json 的非 dependencies 字段 | 如 version / description / keywords |
| git diff 中无任何 .ts/.tsx/.js/.jsx/.vue/.svelte/.py/.java/.go/.rs/.sh 文件 | 确认无运行时代码变更 |

**不满足排除清单 → 必须执行 Tier 3。** 以下场景不再作为跳过理由：
- ❌ "只改了纯函数/工具函数" → 纯函数也有集成边界（调用方、导入链）
- ❌ "只改了配置" → 配置变更可能影响运行时行为
- ❌ "只改了样式" → CSS Module 引用断裂、Tailwind class 冲突可导致渲染崩溃
- ❌ "只改了测试" → 测试变更可能影响 CI 通过率

**检查项**：Dev server 启动成功 | API 端点响应 | 导入路径可解析 | 无运行时 crash

**执行规则**：
- 满足排除清单 → 标记 `Tier 3 健康检查: N/A（<原因>）` + 写 Tier 3 Skip 审计区，直接进入 Step 1
- 不满足排除清单且通过 → 继续 Step 1
- 不满足排除清单但失败 → ❌ 阻塞（环境不就绪，场景无法执行）
- 不满足排除清单但跳过 → **必须**在 QA 报告中写入 `Tier 3 健康检查跳过原因:` 字段 + Tier 3 Skip 审计区 → **阻断 auto-approve，强制 `gate: "review-accept"` 人工审批**

#### Tier 3 Step 1+: 真实场景执行
...
> **教训**：little-bee 鼻字 NoseScene.tsx（UI 组件）验证方案只有数据层测试，Tier 3 全通过但渲染时 framer-motion 崩溃。验证方案必须覆盖核心变更层级。

**验证手段**（按优先级选择，优先产出可复用测试）：

1. **写 E2E 测试文件 → 运行**（优先）
   - 项目有 e2e 框架（Playwright / Cypress / vitest + supertest / Shell 脚本）→ 为本次变更写 e2e 测试文件
   - 测试文件命名：`<feature>.e2e.test.ts` 或 `<feature>.e2e.sh`，放在项目测试目录
   - 运行产出的测试文件，记录通过/失败结果
   - **价值**：测试文件随代码提交，后续可回归复用

2. **跑已有 E2E 测试**（次选）
   - 项目已有覆盖本次变更的 e2e 测试 → 直接运行（`npx playwright test` / `mvn verify -P e2e`）
   - 全部通过 → ✅ | 有失败 → ❌

3. **手动命令验证**（仅作补充，不作为唯一手段）
   - 用 curl / CLI 命令逐场景验证，每个场景记录 `执行:` + `输出:`
   - **铁律**：手动命令验证必须伴随方式 1 或 2 之一。不允许跳过 e2e 测试仅用手动命令
   - 项目无 e2e 框架 → QA 阶段必须搭建最小 e2e 验证（如 `curl` 命令封装为 shell 脚本 `<feature>.e2e.sh`），而非散落的手动 curl

> **判断规则**：项目根目录存在 `playwright.config` / `cypress.config` / `tests/e2e/` / `__tests__/e2e/` / `test/e2e/` 目录 → 视为有 e2e 框架。`package.json` 含 `@playwright/test` / `cypress` 依赖 → 同理。Java 项目有 `src/test/java/**/e2e/` 或 `*IT.java` 文件 → 同理。

**执行规则**：
- 从设计文档的 `## 验证方案 > 真实测试场景` 读取场景列表（经过上述覆盖检查，可能已补充新场景）
- **设计文档必须提供可直接执行的验证命令**（不接受"验证 X 是否正确"等描述性要求）。如果设计文档只写了描述性验证要求，QA 阶段必须先将其转化为具体命令再执行
...
- 超时：单个场景 60s，总计 180s
- 与 Tier 0/Tier 1 的区别：Tier 0/Tier 1 验证「代码是否正确」，Tier 3 验证「功能在真实用户场景下是否可用」

**⚠️ 铁律：证据必须来自运行时**

Tier 3 的证据只接受**运行时输出**（命令 stdout、HTTP response body、浏览器截图、进程退出码）。以下不算证据：
- ❌ "代码逻辑与设计对应" / "计算公式正确" — 这是代码审查，不是场景验证
- ❌ "实现了 useEffect 同步" / "已添加守卫" — 这是实现描述，不是运行结果
- ❌ 任何不含实际命令执行的结论性文字

> **教训**：sdd align-metrics-time-window — 设计文档明确写了"验证 badge 显示正确天数"，Tier 3 报告只写"逻辑对应、公式正确"就标 ✅，没有启动 dev server 也没有验证页面渲染，属于静默跳过。
...
| 借口 | 现实 |
|------|------|
| Tier 1 单测已覆盖 / tsc 通过就够了 | 单测验证逻辑，场景验证真实运行环境；tsc 只保证类型，不保证运行时正确 |
...
**在同一轮响应中使用 Agent 工具启动全部审查 Agent（Tier 4a-4b + 条件专家）。** 所有 Agent 独立运行、互不依赖，完成后合流。
...
#### Tier 4a: design-reviewer Agent（设计符合性）
...
#### Tier 4b: code-quality-reviewer Agent（代码质量）
...
#### Tier 4c-4h: 专家子代理（并行，条件触发）

对每个满足触发条件的专家，使用 Agent 工具（model: "sonnet"）并行启动，prompt 填入：
- 项目根目录路径
- `git diff --stat` 变更摘要

每个专家输出独立的审查报告（N 个问题 + Strengths + Issues）。全部与 Tier 4a/Tier 4b 在同一轮响应中并行启动。
...
如果 Tier 3 Step 0 被标记 N/A（满足排除清单），QA 报告**必须**包含以下审计区：

```markdown
**Tier 3 Skip 审计**
- 排除条件命中: [列出具体满足的排除条件]
- 变更文件清单: [git diff --stat 输出]
- 运行时代码变更: 无 [或列出如果有则说明为什么不排除]
- 结论: Tier 3 N/A / Tier 3 必须执行
```

如果 Tier 3 Step 0 不满足排除清单但执行被跳过（健康检查失败、环境不就绪等），QA 报告**必须**包含：
...
**步骤 1 — 场景完整性检查**：统计 Tier 3 报告中 `执行:` 标记数量 E，对比设计文档验证方案中的实际场景总数 N。E < N → ❌ 有场景被跳过，回去补做 Wave 2 中遗漏的场景。同时验证每个场景是否都包含 `执行:` 和 `输出:` 标记，纯描述性文字视为 ❌。

**步骤 1.5 — Tier 3 执行性检查**：检查 QA 报告中是否存在 Tier 3 Skip 审计区。
- Tier 3 被标记 N/A → 检查排除条件是否合理（变更文件中是否有运行时代码）
  - 有运行时代码但被排除 → ❌ 回去补做 Tier 3
  - 确认排除合理 → 继续
- Tier 3 无 Skip 审计且无执行记录 → ❌ 回去补做 Tier 3（不允许静默跳过）

**步骤 2 — 健康检查覆盖检查**：检查变更分析中是否满足排除清单。不满足排除清单但报告中健康检查标记为跳过且缺少 `Tier 3 健康检查跳过原因:` 字段 → 回去补做。如有跳过原因 → 强制走人工审批（不论其他 Tier 状态）。

- **全部 ✅（可有 ⚠️）** → 更新 frontmatter：`gate: "review-accept"`
- **Tier 3 健康检查声明跳过** → 更新 frontmatter：`gate: "review-accept"`（即便其他全绿，禁止 auto-approve）
- **有 ❌** → 更新 frontmatter：`phase: "auto-fix"`，在报告末尾列出需修复项清单

#### Auto-Approve 处理

如果 frontmatter `auto_approve` 为 `true` 且全部 ✅ 且 Tier 3 健康检查未声明跳过：
- 跳过 `gate: "review-accept"`，直接更新 `phase: "merge"`
- 追加变更日志：QA 全部通过（auto-approve）

如果 Tier 3 健康检查被声明跳过（即便其他全 ✅）：
- 设置 `auto_approve: false`
- 设置 `gate: "review-accept"`，报告中注明「Tier 3 集成健康检查被跳过，需人工确认跳过合理性」
