...
**跳过规则**：
- 触发条件不满足 → 标记 `Tier 3 健康检查: N/A（<原因>）`，直接进入 Step 1
- 触发条件满足且通过 → 继续 Step 1
- 触发条件满足但失败 → ❌ 阻塞（环境不就绪，场景无法执行）
- 触发条件满足但跳过 → **必须**在 QA 报告中写入 `Tier 3 健康检查跳过原因:` 字段 → **阻断 auto-approve，强制 `gate: "review-accept"` 人工审批**
...
3. **手动命令验证**（兜底）
   - 项目无 e2e 框架且写测试文件成本过高 → 用 curl / CLI 命令逐场景验证
   - 每个场景记录 `执行:` + `输出:`
...
**步骤 2 — 健康检查覆盖检查**：检查变更分析中是否满足 Tier 3 Step 0 触发条件。如满足但报告中健康检查标记为跳过且缺少 `Tier 3 健康检查跳过原因:` 字段 → 回去补做。如有跳过原因 → 强制走人工审批（不论其他 Tier 状态）。
