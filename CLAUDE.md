...
### 2026-06-04
- autopilot 升级至 v4.5.2：修复 design.md 缺失 — step 2a 必须写出独立 design.md 文件
  - step 2a：设计文档写入 $TASK_DIR/design.md（独立文件），state.md 仅保留摘要引用
  - step 3 plan-reviewer / implement 蓝队红队：全部指向 $TASK_DIR/design.md
  - HARD-GATE：$TASK_DIR/design.md 必须存在且 ≥3 章节，否则禁止 step 2b
...
- autopilot 升级至 v4.5.1：修复 brainstorm→design 衔接缺失 HARD-GATE，防止跳过设计文档直接写实现计划
  ...
  - brainstorm.md 模板末节重命名："设计决策" → "输入材料"，消除歧义
  - brainstorm SKILL.md 新增显式声明：本文件是输入材料，不是设计文档本身

- autopilot 升级至 v4.5.0：QA Tier 3 触发条件收紧 + 取消手动命令兜底 + Skip 审计强制
  - 触发条件反转：从「4 个窄条件触发」改为「默认执行 + 纯静态排除清单」（仅文档/CI 配置/版本号/无运行时代码变更可排除）
  - 取消手动命令验证兜底：降级路径从三级（写 e2e → 跑已有 e2e → 手动命令）改为两级（写 e2e → 跑已有 e2e），手动命令仅作补充且必须绑定 e2e 测试
  - Tier 3 Skip 审计：N/A 和跳过两种场景都必须在 QA 报告中写入审计区（排除条件命中 / 变更文件清单 / 运行时代码变更 / 结论）
  - 结果判定强化：从两步检查升级为三步（新增步骤 1.5 Tier 3 执行性检查，堵死静默跳过）
  - 3 个文件同步（qa-phase.md / qa-report-template.md / SKILL.md）
...
- autopilot 升级至 v4.2.0：Tier 3 集成验证新增反跳过防护 + 人工审批门
  - Tier 3 从「条件性」升级为「条件触发，触发后不可跳过」
  - 明确 4 类触发条件（前端组件/API 端点/模块导出/服务端启动逻辑）
  - 跳过必须声明原因（QA 报告中写入 `Tier 3 跳过原因:` 字段）
  - 声明跳过阻断 auto-approve，强制走 `gate: "review-accept"` 人工审批
  - 结果判定从两步检查升级为三步（新增步骤 3 Tier 3 覆盖检查）
  - 新增防合理化指南（4 条常见借口 + 反驳）
